...
def load_fashion_mnist(raw=False):
    # 定义数据转换
    ...
    # 下载/加载数据集
    train_set = datasets.FashionMNIST('./data/FashionMNIST/', download=True, train=True, transform=transform)
    test_set = datasets.FashionMNIST('./data/FashionMNIST/', download=True, train=False, transform=transform)
    
    # 如果需要原始数据
    if raw:
        x_raw_train, y_raw_train = train_set.data.numpy(), train_set.targets.numpy()
        x_raw_test, y_raw_test = test_set.data.numpy(), test_set.targets.numpy()
        
        # 可能还需要将数据范围进行归一化到 [0, 1] 或其他你需要的范围
        min_val = np.min(x_raw_train)  # 计算最小值
        max_val = np.max(x_raw_train)  # 计算最大值

        return (x_raw_train, y_raw_train), (x_raw_test, y_raw_test), min_val, max_val
    
    else:
        # 加载为PyTorch DataLoader
...
(x_raw, y_raw), (x_raw_test, y_raw_test), min_, max_ = load_fashion_mnist(raw=True)
...
X_train_party_left, X_train_party_right = X_train_party[:, :, :14], X_train_party[:, :, 14:]
...
poisoned_X_test_left_ch = np.expand_dims(poisoned_X_test_left, axis = 1)
poisoned_X_test_right_ch = np.expand_dims(poisoned_X_test_right, axis = 1)
...
# X_test_left, X_test_right = X_test[:, :, :14], X_test[:, :, 14:]
X_test_pt_left = np.expand_dims(X_test_left, axis = 1)
X_test_pt_right = np.expand_dims(X_test_right, axis = 1)
...
#save
# for fusion_key in fusion_types:
#     torch.save(model_dict[fusion_key], f'{fusion_key}_weights_f.pth')
...
# torch.save(state_dicts_dict, 'party_models_dict_f.pth')
...
#     model_structure.load_state_dict(torch.load(f'{fusion_key}_weights_f.pth',map_location=device))
#     model_dict[fusion_key] = model_structure.state_dict()


# loaded_state_dicts_dict = torch.load('party_models_dict_f.pth',map_location=device)
# party_models_dict = {}
...
# save
# for fusion_key in fusion_types:
#     torch.save(model_dict[fusion_key], f'{fusion_key}_weights_f_n10.pth')
...
# torch.save(state_dicts_dict, 'party_models_dict_f_n10.pth')
...
for fusion_key in fusion_types:
    ...
    model_structure = copy.deepcopy(initial_model)  # 修改为实际模型结构
    model_structure.load_state_dict(torch.load(f'{fusion_key}_weights_f_n10.pth',map_location=device))
    model_dict[fusion_key] = model_structure.state_dict()


loaded_state_dicts_dict = torch.load('party_models_dict_f_n10.pth',map_location=device)
party_models_dict = {}
