...
# #save
# for fusion_key in fusion_types:
#     torch.save(model_dict[fusion_key], f'{fusion_key}_weights.pth')
...
# torch.save(state_dicts_dict, 'party_models_dict.pth')
...
#     model_structure.load_state_dict(torch.load(f'{fusion_key}_weights.pth',map_location=device))
#     model_dict[fusion_key] = model_structure.state_dict()


# loaded_state_dicts_dict = torch.load('party_models_dict.pth',map_location=device)
# party_models_dict = {}
...
#save
# for fusion_key in fusion_types:
#     torch.save(model_dict[fusion_key], f'{fusion_key}_weights_n10.pth')
...
# torch.save(state_dicts_dict, 'party_models_dict_n10.pth')
...
for fusion_key in fusion_types:
    ...
    model_structure = copy.deepcopy(initial_model)  # 修改为实际模型结构
    model_structure.load_state_dict(torch.load(f'{fusion_key}_weights_n10.pth',map_location=device))
    model_dict[fusion_key] = model_structure.state_dict()


loaded_state_dicts_dict = torch.load('party_models_dict_n10.pth',map_location=device)
party_models_dict = {}
...
for fusion_key in fusion_types:
    ...
    model_ref_vec = (num_parties / ((num_parties - 1) * (1 - alpha))) * nn.utils.parameters_to_vector(fedavg_model.parameters()) \
               - (1 - alpha) / ((num_parties - 1) * (1 - alpha)) * nn.utils.parameters_to_vector(party0_model.parameters())
