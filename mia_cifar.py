...
def args_parser():
    ...
    parser.add_argument('--dataroot', type=str, default="../data/CIFAR10",
                        metavar='PATH',
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        # trainset = datasets.FashionMNIST('./data/FashionMNIST/', download=True, train=True, transform=trans_mnist)
        # testset = datasets.FashionMNIST('./data/FashionMNIST/', download=True, train=False, transform=trans_mnist)
        trans_cifar10 = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
        ])

        trainset = datasets.CIFAR10('./data/CIFAR10/', download=True, train=True, transform=trans_cifar10)
        testset = datasets.CIFAR10('./data/CIFAR10/', download=True, train=False, transform=trans_cifar10)
    ...
    #     path = F"infattack_10/resnet-shadow_model_{i}_cifar.pt"
    ...
    # xpath = f"infattack_10/resnet_attack_x_{args.unlearning_class}_cifar.pt"
    # ypath = f"infattack_10/resnet_attack_y_{args.unlearning_class}_cifar.pt"
    # torch.save(tensor_x, xpath)
    # torch.save(tensor_y, ypath)

    # Load datasets
    tensor_x = torch.load(f"infattack_10/resnet_attack_x_{args.unlearning_class}_cifar.pt")
    tensor_y = torch.load(f"infattack_10/resnet_attack_y_{args.unlearning_class}_cifar.pt")
    ...
    # # Save attack model
    # path = F"infattack_10/resnet_attack_model_{args.unlearning_class}_cifar.pt"
...
#################################################################
    ...
    #                 party_models.append(AlexNetCIFAR())
    #             else:
    #                 model = copy.deepcopy(current_model)
    #                 model_update= local_training.train(model=model, 
    ...
    # #save
    # for fusion_key in fusion_types:
    #     path = F"infattack_10/net_glob_{fusion_key}_cifar.pt" 
    ...
    # path = F"infattack_10/party_models_dict_cifar.pt" 
